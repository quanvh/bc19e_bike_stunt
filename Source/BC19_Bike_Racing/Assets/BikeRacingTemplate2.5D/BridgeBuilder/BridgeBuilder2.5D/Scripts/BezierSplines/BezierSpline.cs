...
namespace Kamgam.BridgeBuilder25D
{
	...
	public partial class BezierSpline : MonoBehaviour
	{
		...
		public bool EditMode = true; // only used in Editor
		...
		*/
		...
		public bool HasTransformChanged()
		{
			...
			for (int i = 0; i < transform.childCount; i++)
			{
				...
				if (child.hasChanged && child.name.StartsWith("Point "))
				{
					changed = true;
		...
		public bool HasPointCountChanged()
		{
			...
			for (int i = 0; i < transform.childCount; i++)
			{
				if (transform.GetChild(i).gameObject.activeSelf && transform.name.StartsWith("Point "))
					pointCount++;
			}
		...
		public List<Vector3> GetSegmentPoints(float size = 0.1f, int resolution = 100, float variationPercentage = 0f)
		{
			List<Vector3> segmentPoints = new List<Vector3>();

			float rndSize = (size + UnityEngine.Random.Range(-variationPercentage * 0.5f, variationPercentage * 0.5f));
			float sqrSize = rndSize * rndSize;
			...
			for (int i = 1; i <= resolution; i++)
			{
				...
				if (Vector3.SqrMagnitude(candidate1 - current) >= sqrSize)
				{
					current = candidate0;
					segmentPoints.Add(current);
					rndSize = (size + UnityEngine.Random.Range(-variationPercentage * 0.5f, variationPercentage * 0.5f));
					sqrSize = rndSize * rndSize;
				}
			...
			{
				// stretch
				segmentPoints[segmentPoints.Count - 1] = Points[Points.Count - 1].Position;
			}
			else
			{
				// add final segment
				segmentPoints.Add(Points[Points.Count - 1].Position);
			}
