...
namespace Kamgam.BikeAndCharacter25D.Helpers
{
    public static class ExtensionsForCollections
    {
        ...
        {
            int indexA = source.IndexOf(itemA);
            int indexB = source.IndexOf(itemB);
        ...
        {
            int index = source.IndexOf(itemB);
            if (index >= 0)
                source.Swap(indexA, index);
        }

        public static void Swap<T>(this IList<T> source, T itemA, int indexB)
        {
            int index = source.IndexOf(itemA);
            if(index >= 0)
                source.Swap(index, indexB);
        }
