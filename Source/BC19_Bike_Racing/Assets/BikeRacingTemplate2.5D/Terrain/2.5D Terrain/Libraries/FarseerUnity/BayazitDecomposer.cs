...
// Original Source: https://github.com/gabstv/Farseer-Unity3D/blob/master/LICENSE
// License: Microsoft Permissive License (Ms-PL)

// Notes (Kamgam): Does cause infinite loop for some self intersecting polygons!
...
namespace Kamgam.Farseer
{
    ...
    public static class BayazitDecomposer
    {
        private static Vector2 At(int i, List<Vector2> vertices)
        {
            ...
            return vertices[Mathf.Clamp(i < 0 ? s - (-i % s) : i % s, 0, vertices.Count-1)];
        }
