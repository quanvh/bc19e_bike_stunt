...
// signal to other socde that this lib exists
#define clipper_lib

using System;
using System.Collections.Generic;

namespace Kamgam.Terrain25DLib.ClipperLib
{
  ...
  //assumes that e1 will be to the Right of e2 ABOVE the intersection
    ...
    private void JoinCommonEdges()
          ...
          IntPoint pt2, IntPoint pt3, double distSqrd)
      {
        ...
        /// <summary>
        /// Modified version of SlopesNearCollinear to avoid spikes.
        /// </summary>
        ...
        /// <returns></returns>
        private static bool SlopesNearCollinearForCleaning(IntPoint pt1,
         IntPoint pt2, IntPoint pt3, double distSqrd)
        {
            //this function is more accurate when the point that's GEOMETRICALLY 
            //between the other 2 points is the one that's tested for distance.  
            //nb: with 'spikes', either pt1 or pt3 is geometrically between the other pts
            //
            //Notes by kagam: to avoid false positives we forcefully return false for 'spikes'.
            if (Math.Abs(pt1.X - pt2.X) > Math.Abs(pt1.Y - pt2.Y))
            {
                if ((pt1.X > pt2.X) == (pt1.X < pt3.X))
                    return false;
      ...
      public static Path CleanPolygon(Path path, double distance = 1.415, bool mergeStraightEdges = true)
      {
        ...
        while (op.Idx == 0 && op.Next != op.Prev)
        {
          ...
          */
          // modified version
          else if (mergeStraightEdges && SlopesNearCollinearForCleaning(op.Prev.Pt, op.Pt, op.Next.Pt, distSqrd / (distSqrd * 0.3)))
          {
            op = ExcludeOp(op);
            cnt--;
          }
      ...
      public static Paths CleanPolygons(Paths polys,
          double distance = 1.415, bool mergeStraightEdges = true)
      {
        Paths result = new Paths(polys.Count);
        for (int i = 0; i < polys.Count; i++)
          result.Add(CleanPolygon(polys[i], distance, mergeStraightEdges));
        return result;
      }
      //------------------------------------------------------------------------------
