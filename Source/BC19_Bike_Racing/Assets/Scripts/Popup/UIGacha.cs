...
namespace Bacon
{
    public class UIGacha : PopupBase
    {
        ...
        private void SetAdsImage()
        {
            ...
            else
            {
                ...
                {
                    iconVideo.SetActive(!CarModel.FreeSpinCustom);
                }
                if (CurrentState == CUSTOMIZE_SELECT.Char || CurrentState == CUSTOMIZE_SELECT.Helmet)
                {
                    iconVideo.SetActive(!CustomModel.FreeSpinCustom);
                }
        ...
        public void OnSpin()
        {
            ...
            else
            {
                ...
                {
                    AdsController.Instance.ShowReward(ADTYPE.SPIN_CAR);
                }
                else if ((CurrentState == CUSTOMIZE_SELECT.Char)
                    && !CustomModel.FreeSpinCustom)
                {
                    AdsController.Instance.ShowReward(ADTYPE.SPIN_CHAR);
                }
                else if (CurrentState == CUSTOMIZE_SELECT.Helmet
                    && !CustomModel.FreeSpinCustom)
                {
                    AdsController.Instance.ShowReward(ADTYPE.SPIN_HELMET);
                }
                else
                {
                    OnSpinCustom();
            ...
            else
            {
                ...
                if (Reward && RewardPanel)
                {
                    if (imgThumb)
                    {
                        if (_reward.TargetType == REWARD_TYPE.CHARACTER)
                        {
                            imgThumb.transform.localScale = 2.3f * Vector3.one;
                        }
                        else if (_reward.Type == REWARD_TYPE.GOLD)
                        {
                            imgThumb.transform.localScale = 1.6f * Vector3.one;
                        }
                        else
                        {
        ...
        public void OnClose()
        {
            ...
            if (DataManager.Instance.isLuckySpinCustom)
            {
                if (CurrentState == CUSTOMIZE_SELECT.Car)
                {
                    GarageController.Instance.OnCarLoaded?.Invoke(CarModel.ID);
                }
                else
                {
                    GarageController.Instance.OnCustomLoaded?.Invoke(CustomModel);
                }
            }
